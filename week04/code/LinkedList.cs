...
public class LinkedList : IEnumerable<int>
{
    ...
    {
        // Search for the node that matches 'value' by starting at the head
        Node? curr = _head;
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                // If the node to remove is the head, use RemoveHead
                if (curr == _head)
                {
                    RemoveHead();
                }
                // If the node to remove is the tail, use RemoveTail
                else if (curr == _tail)
                {
                    RemoveTail();
                }
                // If the node is in the middle, reconnect the surrounding nodes
                else
                {
    ...
    {
        // Search through the entire list starting at the head
        Node? curr = _head;
        while (curr is not null)
        {
            // If we find a match, replace the value
